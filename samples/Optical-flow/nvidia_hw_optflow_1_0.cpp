...
int main(int argc, char** argv)
{
    ...
    const int gpuId = 0;
    cv::Ptr<cv::cuda::NvidiaOpticalFlow_1_0> d_optFlow = 
        cv::cuda::NvidiaOpticalFlow_1_0::create(player->frame0().size(), cv::cuda::NvidiaOpticalFlow_1_0::NV_OF_PERF_LEVEL_MEDIUM,
        false, false, false, gpuId, d_stream1, d_stream2);
    cv::cuda::GpuMat d_gray, d_prevGray, d_optFlowImage, d_optFlowUpsImage;
    cv::Mat gray;
    ...
    while ( loop )
    {
        ...
        *player >> frame;
        ...
        cv::Mat optFlowImage;
        {
            ...
            d_optFlow->upSampler(d_optFlowImage, gray.size(), d_optFlow->getGridSize(), d_optFlowUpsImage);
            d_optFlowUpsImage.download(optFlowImage);
        }
